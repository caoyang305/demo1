<template>
  <div class="starbox" :stars="stars()">
    <template v-if="astar == 0">
      <span>暂无评分</span>
    </template>
    <template v-else>
      <template v-for="n in astar">
        <span class="astar"></span>
      </template>
      <template v-for="n in bstar">
        <span class="bstar"></span>
      </template>
      <template v-for="n in nstar">
        <span class="nstar"></span>
      </template>
      <span class="aver">{{rating.average}}</span>
    </template>
  </div>
</template>
<script>
export default {
  ...
  methods:{
    stars () {
      var aver = this.rating.average
      this.astar = parseInt(aver / 2)
      this.bstar = aver % 2 == 0 ? 0 : 1
      this.nstar = 5 - this.astar - this.bstar
    }
  }
}
</script>
